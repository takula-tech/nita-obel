...
/// By default, Rust unwinds the stack when a panic happens
/// This CatchUnwind future is a safety net around another future. Its job is to:
/// Safely run an async operation.
/// Catch any panics that might happen inside the operation.
/// Return either the operation's result or the panic, without crashing the entire program.
/// This kind of thing is super useful in async runtimes because a panic in one task shouldn’t necessarily take down the whole system. This lets you handle those situations gracefully.
