...
use obel_platform::collections::HashTable;
...
///
/// ```
/// use obel_reflect::{PartialReflect, Reflect, Map};
...
impl Map for DynamicMap {
    ...
    fn get_mut(&mut self, key: &dyn PartialReflect) -> Option<&mut dyn PartialReflect> {
        let hash = Self::internal_hash(key);
        let eq = Self::internal_eq(key, &self.values);
        self.indices.find(hash, eq).map(|&index| &mut *self.values[index].1)
    }

    fn get_at(&self, index: usize) -> Option<(&dyn PartialReflect, &dyn PartialReflect)> {
        self.values.get(index).map(|(key, value)| (&**key, &**value))
    }
    ...
    fn get_at_mut(
        ...
        self.values.get_mut(index).map(|(key, value)| (&**key, &mut **value))
    }
...
impl_type_path!((in obel_reflect) DynamicMap);
...
mod tests {
    ...
    fn test_into_iter() {
        ...
        for (index, item) in map.into_iter().enumerate() {
            let key = item.0.try_take::<usize>().expect("couldn't downcast to usize");
            let value = item.1.try_take::<String>().expect("couldn't downcast to String");
            assert_eq!(index, key);
    ...
    fn test_map_get_at() {
        ...
        let value = value_r.try_downcast_ref::<String>().expect("Couldn't downcast to String");
        let key = key_r.try_downcast_ref::<usize>().expect("Couldn't downcast to usize");
    ...
    fn test_map_get_at_mut() {
        ...
        let value = value_r.try_downcast_mut::<String>().expect("Couldn't downcast to String");
        let key = key_r.try_downcast_ref::<usize>().expect("Couldn't downcast to usize");
        assert_eq!(key, &1usize);
