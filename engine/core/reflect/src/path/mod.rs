...
pub trait ReflectPath<'a>: Sized {
    ...
    fn element<T: Reflect>(self, root: &dyn PartialReflect) -> PathResult<'a, &T> {
        self.reflect_element(root)
            .and_then(|p| p.try_downcast_ref::<T>().ok_or(ReflectPathError::InvalidDowncast))
    }
    ...
    fn element_mut<T: Reflect>(self, root: &mut dyn PartialReflect) -> PathResult<'a, &mut T> {
        self.reflect_element_mut(root)
            .and_then(|p| p.try_downcast_mut::<T>().ok_or(ReflectPathError::InvalidDowncast))
    }
}
...
/// ### Example
/// ```
/// # use obel_reflect::{GetPath, Reflect};
...
/// ```
/// # use std::borrow::Cow;
/// # use obel_reflect::access::Access;
/// # use obel_reflect::ParsedPath;
/// let path_elements = [
...
impl ParsedPath {
    ...
    /// # Example
    /// ```
    /// # use obel_reflect::{ParsedPath, Reflect, ReflectPath};
    /// #[derive(Reflect)]
    /// struct Foo {
    ///   bar: Bar,
    /// }
...
mod tests {
    ...
    fn invalid_access(
        ...
        ReflectPathError::InvalidAccess(AccessError {
            kind: AccessErrorKind::IncompatibleTypes {
                actual,
                expected,
            },
    ...
    #[test]
    fn try_from() {
        assert_eq!(ParsedPath::try_from("w").unwrap().0, &[offset(access_field("w"), 1)]);

        let r = ParsedPath::try_from("w[");
        let matches = matches!(r, Err(ReflectPathError::ParseError { .. }));
        assert!(matches, "ParsedPath::try_from did not return a ParseError for \"w[\"");
    }

    #[test]
    fn parsed_path_parse() {
        assert_eq!(ParsedPath::parse("w").unwrap().0, &[offset(access_field("w"), 1)]);
        assert_eq!(
        ...
        assert_eq!(
            ...
            &[offset(access_field("x"), 1), offset(Access::FieldIndex(0), 2)]
        );
        assert_eq!(
        ...
        *a.path_mut::<u32>("tuple_variant.0").unwrap() = 1337;
        ...
        assert_eq!(
            ...
            ReflectPathError::InvalidAccess(AccessError {
                ...
                access: ParsedPath::parse_static("unit_variant.0").unwrap()[1].access.clone(),
                offset: Some(13),
            })
        );
        assert_eq!(
    ...
    fn accept_leading_tokens() {
        ...
        assert_eq!(
            ...
            &[offset(Access::FieldIndex(0), 1), offset(access_field("foo"), 3)]
        );
        assert_eq!(ParsedPath::parse(".5").unwrap().0, &[offset(Access::TupleIndex(5), 1)]);
        assert_eq!(
            ParsedPath::parse("[0].łørđ").unwrap().0,
            &[offset(Access::ListIndex(0), 1), offset(access_field("łørđ"), 4)]
        );
    }
}
