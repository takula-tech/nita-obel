...
///
/// [`Reflect` derive macro]: obel_reflect_derive::Reflect
...
impl Generics {
    ...
    pub fn with(mut self, info: impl Into<GenericInfo>) -> Self {
        self.0 = IntoIterator::into_iter(self.0).chain(core::iter::once(info.into())).collect();
        self
    }
}
...
impl TypeParamInfo {
    ...
    ///
    /// ```
    /// # use obel_reflect::{GenericInfo, Reflect, Typed};
    /// #[derive(Reflect)]
    /// struct Foo<T = f32>(T);
    ///
...
mod tests {
    ...
    fn should_maintain_order() {
        ...
        let generics =
            <Test<f32, String, 10> as Typed>::type_info().as_tuple_struct().unwrap().generics();
    ...
    fn should_get_by_name() {
        ...
        let generics = <Test<f32, String, 10> as Typed>::type_info().as_enum().unwrap().generics();
    ...
    fn should_store_defaults() {
        ...
        let generics = <Test<f32> as Typed>::type_info().as_tuple_struct().unwrap().generics();
