...
mod tests {
    ...
    fn should_get_enum_type_info() {
        ...
        if let TypeInfo::Enum(info) = info {
            ...
            assert_eq!(MyEnum::short_type_path(), info.type_path_table().short_path());
            ...
            if let VariantInfo::Tuple(variant) = info.variant("B").unwrap() {
                ...
                assert!(variant.field_at(0).unwrap().type_info().unwrap().is::<usize>());
                assert!(variant.field_at(1).unwrap().type_info().unwrap().is::<i32>());
            } else {
            ...
            if let VariantInfo::Struct(variant) = info.variant("C").unwrap() {
                ...
                assert!(variant.field("foo").unwrap().type_info().unwrap().is::<f32>());
            } else {
    ...
    fn dynamic_enum_should_apply_dynamic_enum() {
        ...
        assert!(a.reflect_partial_eq(&b).unwrap_or_default(), "dynamic enums should be equal");

        a.set_variant("A", ());
        assert!(!a.reflect_partial_eq(&b).unwrap_or_default(), "dynamic enums should not be equal");
    ...
    fn enum_should_return_correct_variant_path() {
        // === Unit === //
        let value = MyEnum::A;
        assert_eq!("obel_reflect::enums::tests::MyEnum::A", value.variant_path());

        // === Tuple === //
        let value = MyEnum::B(0, 0);
        assert_eq!("obel_reflect::enums::tests::MyEnum::B", value.variant_path());
        ...
        assert_eq!("obel_reflect::enums::tests::MyEnum::C", value.variant_path());
    }
    ...
    fn enum_should_apply() {
        ...
        assert!(value.reflect_partial_eq(&MyEnum::B(123, 321)).unwrap_or_default());

        // === MyEnum::B -> MyEnum::B === //
        value.apply(&MyEnum::B(321, 123));
        assert!(value.reflect_partial_eq(&MyEnum::B(321, 123)).unwrap_or_default());
    ...
    fn enum_should_partial_eq() {
        ...
        let a: &dyn PartialReflect = &TestEnum::C {
            value: 123,
        };
        let b: &dyn PartialReflect = &TestEnum::C {
            value: 123,
        };
        assert!(
            a.reflect_partial_eq(b).unwrap_or_default(),
        ...
        let b: &dyn PartialReflect = &TestEnum::C1 {
            value: 123,
        };
        assert!(
            !a.reflect_partial_eq(b).unwrap_or_default(),
        ...
        let b: &dyn PartialReflect = &TestEnum::C2 {
            value: 1.23,
        };
        assert!(
            !a.reflect_partial_eq(b).unwrap_or_default(),
