use crate::Reflect;
use alloc::boxed::Box;
use obel_platform::collections::{HashMap, hash_map::Iter};
...
impl SerializationData {
    ...
    /// ```
    /// # use core::any::TypeId;
    /// # use obel_reflect::{Reflect, Struct, TypeRegistry, serde::SerializationData};
    /// #[derive(Reflect)]
    /// struct MyStruct {
    ///   serialize_me: i32,
    ...
    pub fn generate_default(&self, index: usize) -> Option<Box<dyn Reflect>> {
        self.skipped_fields.get(&index).map(SkippedField::generate_default)
    }
