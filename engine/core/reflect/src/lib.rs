...
#[cfg(test)]
#[expect(clippy::approx_constant, reason = "We don't need the exact value of Pi here.")]
mod tests {
    ...
    fn reflect_map() {
        ...
        assert_eq!(10, *map.get(&key_b).unwrap().try_downcast_ref::<u32>().unwrap());
        assert!(map.get(&key_c).is_none());
        *map.get_mut(&key_b).unwrap().try_downcast_mut::<u32>().unwrap() = 20;
        assert_eq!(20, *map.get(&key_b).unwrap().try_downcast_ref::<u32>().unwrap());
    }
    ...
    fn reflect_unit_struct() {
        ...
        patch.insert(3u32);
        patch.insert(4u64);
        assert_eq!(3, *patch.field(0).unwrap().try_downcast_ref::<u32>().unwrap());
        assert_eq!(4, *patch.field(1).unwrap().try_downcast_ref::<u64>().unwrap());
    ...
    #[test]
    #[should_panic(
        expected = "the given key of type `obel_reflect::tests::Foo` does not support hashing"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "the dynamic type `obel_reflect::DynamicStruct` (representing `obel_reflect::tests::Foo`) does not support hashing"
    )]
    ...
    #[test]
    #[should_panic(
        expected = "the dynamic type `obel_reflect::DynamicStruct` does not support hashing"
    )]
    ...
    fn reflect_ignore() {
        ...
        let values: Vec<u32> =
            foo.iter_fields().map(|value| *value.try_downcast_ref::<u32>().unwrap()).collect();
    ...
    fn should_reflect_clone_with_clone() {
        ...
        let value = Foo(123);
        let clone = value.reflect_clone().expect("should reflect_clone tuple struct");
        assert_eq!(value, clone.take::<Foo>().unwrap());
        ...
        let value = Baz::Unit;
        let clone = value.reflect_clone().expect("should reflect_clone unit variant");
        assert_eq!(value, clone.take::<Baz>().unwrap());

        let value = Baz::Tuple(123);
        let clone = value.reflect_clone().expect("should reflect_clone tuple variant");
        assert_eq!(value, clone.take::<Baz>().unwrap());

        let value = Baz::Struct {
            value: 123,
        };
        let clone = value.reflect_clone().expect("should reflect_clone struct variant");
        assert_eq!(value, clone.take::<Baz>().unwrap());
    }
    ...
    fn should_composite_reflect_clone() {
        ...
        enum MyEnum {
            Unit,
            Tuple(Foo, #[reflect(ignore, clone)] Bar, #[reflect(clone = "clone_baz")] Baz),
            Struct {
                foo: Foo,
    ...
    fn should_call_from_reflect_dynamically() {
        ...
        let reflected =
            rfr.from_reflect(&dynamic_struct).expect("the type should be properly reflected");
        ...
        let not_expected = MyStruct {
            foo: 321,
        };
        assert!(
            !not_expected.reflect_partial_eq(reflected.as_partial_reflect()).unwrap_or_default()
        );
    }
    ...
    fn should_not_auto_register_existing_types() {
        ...
        assert!(
            registry.get_type_data::<ReflectDefault>(TypeId::of::<Bar>()).is_some(),
    ...
    fn reflect_take() {
        ...
        let x: Box<dyn Reflect> = Box::new(Bar {
            x: 2,
        });
    ...
    fn reflect_type_path() {
        ...
        assert_type_paths! {
            Derive => "obel_reflect::tests::Derive", "Derive",
            DerivePath => "my_alias::DerivePath", "DerivePath",
            DerivePathName => "my_alias::MyDerivePathName", "MyDerivePathName",
            DeriveG<Param> => "obel_reflect::tests::DeriveG<obel_reflect::tests::Param>", "DeriveG<Param>",
            DerivePathG<Param, 10> => "my_alias::DerivePathG<obel_reflect::tests::Param, 10>", "DerivePathG<Param, 10>",
            DerivePathNameG<Param> => "my_alias::MyDerivePathNameG<obel_reflect::tests::Param>", "MyDerivePathNameG<Param>",
            Macro => "my_alias::Macro", "Macro",
            MacroName => "my_alias::MyMacroName", "MyMacroName",
            MacroG<Param, 10> => "my_alias::MacroG<obel_reflect::tests::Param, 10>", "MacroG<Param, 10>",
            MacroNameG<Param> => "my_alias::MyMacroNameG<obel_reflect::tests::Param>", "MyMacroNameG<Param>",
        }
    }
    ...
    fn reflect_type_info() {
        ...
        // TypeInfo (unsized)
        assert_eq!(TypeId::of::<dyn Reflect>(), <dyn Reflect as Typed>::type_info().type_id());
        ...
        let value: &dyn Reflect = &MyStruct {
            foo: 123,
            bar: 321,
        };
        let info = value.reflect_type_info();
        ...
        if let TypeInfo::Struct(info) = info {
            ...
            assert_eq!(DynamicStruct::type_path(), info.field("foo").unwrap().type_path());
            assert_eq!("foo", info.field("foo").unwrap().name());
    ...
    fn get_represented_kind_info() {
        ...
        let _: &TupleStructInfo = dyn_tuple_struct.get_represented_tuple_struct_info().unwrap();
    ...
    mod docstrings {
        ...
        fn should_contain_docs() {
            ...
            impl_reflect_opaque!(
                ...
                (in obel_reflect::tests) SomePrimitive
            );
...
obel_reflect::tests::Test {
    ...
    a_struct: obel_reflect::tests::SomeStruct {
        foo: "A Struct!",
    },
    a_tuple_struct: obel_reflect::tests::SomeTupleStruct(
        "A Tuple Struct!",
    ),
    enum_unit: A,
    enum_tuple: B(
    ...
    fn can_opt_out_type_path() {
        ...
        assert_eq!(
            "obel_reflect::tests::can_opt_out_type_path::Foo<obel_reflect::tests::can_opt_out_type_path::NotTypePath>",
            path
        );
        ...
        assert_eq!("Foo<NotTypePath>", registration.type_info().type_path_table().short_path());
    }
    ...
    fn dynamic_types_debug_format() {
        ...
        assert_eq!(
            format!("{:?}", test_struct),
            "DynamicStruct(obel_reflect::tests::TestStruct { \
                tuple: DynamicTuple((0, 1)), \
                tuple_struct: DynamicTupleStruct(obel_reflect::tests::TestTupleStruct(8)), \
    ...
    fn should_reflect_nested_remote_enum() {
        ...
        assert!(matches!(data, MyOuter(external_crate::TheirOuter::<i32>::Unit)));
        data.apply(&patch);
        assert!(matches!(
            data,
    ...
    fn should_take_remote_type() {
        ...
        let output: external_crate::TheirType =
            input.take().expect("should downcast to `external_crate::TheirType`");
        assert_eq!(
    ...
    fn should_try_take_remote_type() {
        ...
        let output: external_crate::TheirType =
            input.try_take().expect("should downcast to `external_crate::TheirType`");
        assert_eq!(
            external_crate::TheirType {
                value: "Hello".to_string(),
            },
    ...
    fn should_take_nested_remote_type() {
        ...
        let output: external_crate::TheirOuter<i32> =
            input.take().expect("should downcast to `external_crate::TheirOuter`");
        assert_eq!(
            ...
            let data = r#"
{
    "glam::Quat": (1.0, 2.0, 3.0, 4.0),
            ...
            let dynamic_struct = de.deserialize(&mut deserializer).expect("Failed to deserialize");
            ...
            let data = r#"
{
    "glam::Vec3": (12.0, 3.0, -6.9),
        ...
        fn vec3_path_access() {
            ...
            assert_eq!(*v.reflect_path("x").unwrap().try_downcast_ref::<f32>().unwrap(), 1.0);
