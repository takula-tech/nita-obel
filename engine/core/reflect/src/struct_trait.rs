...
///
/// ```
/// use obel_reflect::{PartialReflect, Reflect, Struct};
///
/// #[derive(Reflect)]
/// struct Foo {
///     bar: u32,
...
///
/// ```
/// use obel_reflect::{GetField, Reflect};
///
/// #[derive(Reflect)]
/// struct Foo {
///     bar: String,
...
impl<S: Struct> GetField for S {
    fn get_field<T: Reflect>(&self, name: &str) -> Option<&T> {
        self.field(name).and_then(|value| value.try_downcast_ref::<T>())
    }

    fn get_field_mut<T: Reflect>(&mut self, name: &str) -> Option<&mut T> {
        self.field_mut(name).and_then(|value| value.try_downcast_mut::<T>())
    }
}
...
impl_type_path!((in obel_reflect) DynamicStruct);
...
pub fn struct_debug(dyn_struct: &dyn Struct, f: &mut Formatter<'_>) -> core::fmt::Result {
    let mut debug = f.debug_struct(
        dyn_struct.get_represented_type_info().map(TypeInfo::type_path).unwrap_or("_"),
    );
    for field_index in 0..dyn_struct.field_len() {
        let field = dyn_struct.field_at(field_index).unwrap();
        debug.field(dyn_struct.name_at(field_index).unwrap(), &field as &dyn Debug);
    }
    debug.finish()
}
