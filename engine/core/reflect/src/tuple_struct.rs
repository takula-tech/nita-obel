...
///
/// ```
/// use obel_reflect::{PartialReflect, Reflect, TupleStruct};
///
/// #[derive(Reflect)]
/// struct Foo(u32);
///
...
///
/// ```
/// use obel_reflect::{GetTupleStructField, Reflect};
///
/// #[derive(Reflect)]
/// struct Foo(String);
///
...
impl<S: TupleStruct> GetTupleStructField for S {
    fn get_field<T: Reflect>(&self, index: usize) -> Option<&T> {
        self.field(index).and_then(|value| value.try_downcast_ref::<T>())
    }

    fn get_field_mut<T: Reflect>(&mut self, index: usize) -> Option<&mut T> {
        self.field_mut(index).and_then(|value| value.try_downcast_mut::<T>())
    }
}
...
impl_type_path!((in obel_reflect) DynamicTupleStruct);
...
pub fn tuple_struct_debug(
    ...
    let mut debug = f.debug_tuple(
        dyn_tuple_struct.get_represented_type_info().map(TypeInfo::type_path).unwrap_or("_"),
    );
