...
use obel_platform::collections::{HashTable, hash_table::OccupiedEntry as HashTableOccupiedEntry};
...
///
/// ```
/// use obel_reflect::{PartialReflect, Set};
...
impl Set for DynamicSet {
    ...
    fn clone_dynamic(&self) -> DynamicSet {
        ...
        self.hash_table.iter().map(|value| value.clone_value()).for_each(|value| {
    ...
    fn insert_boxed(&mut self, value: Box<dyn PartialReflect>) -> bool {
        ...
        match self.hash_table.find_mut(Self::internal_hash(&*value), Self::internal_eq(&*value)) {
            Some(old) => {
                *old = value;
                false
            }
    ...
    fn contains(&self, value: &dyn PartialReflect) -> bool {
        self.hash_table.find(Self::internal_hash(value), Self::internal_eq(value)).is_some()
    }
}
...
impl_type_path!((in obel_reflect) DynamicSet);
...
impl IntoIterator for DynamicSet {
    ...
    type IntoIter = obel_platform::collections::hash_table::IntoIter<Self::Item>;
...
impl<'a> IntoIterator for &'a DynamicSet {
    ...
    type IntoIter = core::iter::Map<
        obel_platform::collections::hash_table::Iter<'a, Box<dyn PartialReflect>>,
...
mod tests {
    ...
    fn test_into_iter() {
        ...
        for item in set.into_iter() {
            let value = item.try_take::<String>().expect("couldn't downcast to String");
            let index = expected
                .iter()
