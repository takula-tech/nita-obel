...
///
/// ```
/// # use obel_reflect::{Reflect, Typed, TypeInfo};
...
impl CustomAttributes {
    ...
    pub fn iter(&self) -> impl ExactSizeIterator<Item = (&TypeId, &dyn Reflect)> {
        self.attributes.iter().map(|(key, value)| (key, value.reflect_value()))
    }
...
mod tests {
    ...
    fn should_get_custom_attribute_dynamically() {
        ...
        assert!(value.reflect_partial_eq(&String::from("Hello, World!")).unwrap());
    }
    ...
    fn should_debug_custom_attributes() {
        ...
        let attributes = CustomAttributes::default().with_attribute(Foo {
            value: 42,
        });

        let debug = format!("{:?}", attributes);

        assert_eq!(r#"{obel_reflect::attributes::tests::Foo { value: 42 }}"#, debug);
    }
