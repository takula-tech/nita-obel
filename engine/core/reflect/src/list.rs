...
///
/// ```
/// use obel_reflect::{PartialReflect, Reflect, List};
///
...
impl_type_path!((in obel_reflect) DynamicList);
...
impl<T: PartialReflect> FromIterator<T> for DynamicList {
    fn from_iter<I: IntoIterator<Item = T>>(values: I) -> Self {
        values.into_iter().map(|field| Box::new(field).into_partial_reflect()).collect()
    }
}
...
mod tests {
    ...
    fn test_into_iter() {
        ...
        for (index, item) in items.into_iter().enumerate() {
            let value = item.try_take::<usize>().expect("couldn't downcast to usize");
