//! Helpers for working with obel reflection.
...
/// ```
/// # use core::any::Any;
/// # use obel_reflect::{DynamicTypePath, NamedField, PartialReflect, Reflect, ReflectMut, ReflectOwned, ReflectRef, StructInfo, Typed, TypeInfo, TypePath, ApplyError};
/// use obel_reflect::utility::NonGenericTypeInfoCell;
///
/// struct Foo {
///     bar: i32
/// }
...
/// ```
/// # use core::any::Any;
/// # use obel_reflect::{DynamicTypePath, PartialReflect, Reflect, ReflectMut, ReflectOwned, ReflectRef, TupleStructInfo, Typed, TypeInfo, TypePath, UnnamedField, ApplyError, Generics, TypeParamInfo};
/// use obel_reflect::utility::GenericTypeInfoCell;
///
/// struct Foo<T>(T);
///
...
/// ```
/// # use core::any::Any;
/// # use obel_reflect::TypePath;
/// use obel_reflect::utility::GenericTypePathCell;
///
/// struct Foo<T>(T);
///
/// impl<T: TypePath> TypePath for Foo<T> {
...
impl<T: TypedProperty> GenericTypeCell<T> {
    ...
    fn get_by_type_id(&self, type_id: TypeId) -> Option<&T::Stored> {
        self.0.read().unwrap_or_else(PoisonError::into_inner).get(&type_id).copied()
    }
