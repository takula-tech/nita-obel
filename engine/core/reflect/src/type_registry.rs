...
use obel_platform::collections::TypeIdMap;
use obel_platform::utils::{Ptr, PtrMut};
use obel_platform::{
...
impl Debug for TypeRegistryArc {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        self.internal.read().unwrap_or_else(PoisonError::into_inner).type_path_to_id.keys().fmt(f)
    }
}
...
impl TypeRegistry {
    ...
    /// ```
    /// # use core::any::TypeId;
    /// # use obel_reflect::{Reflect, TypeRegistry, std_traits::ReflectDefault};
    ...
    ///
    /// ```
    /// # use obel_reflect::{Reflect, TypeRegistry};
    /// # use core::any::TypeId;
    /// #
    /// # let mut type_registry = TypeRegistry::default();
    /// #
    ...
    fn register_internal(
        ...
        use obel_platform::collections::hash_map::Entry;
    ...
    /// # Example
    /// ```
    /// use obel_reflect::{TypeRegistry, ReflectSerialize, ReflectDeserialize};
    ///
    /// let mut type_registry = TypeRegistry::default();
    ...
    pub fn get_with_type_path(&self, type_path: &str) -> Option<&TypeRegistration> {
        self.type_path_to_id.get(type_path).and_then(|id| self.get(*id))
    }
    ...
    pub fn get_with_type_path_mut(&mut self, type_path: &str) -> Option<&mut TypeRegistration> {
        self.type_path_to_id.get(type_path).cloned().and_then(move |id| self.get_mut(id))
    }
    ...
    pub fn get_with_short_type_path(&self, short_type_path: &str) -> Option<&TypeRegistration> {
        self.short_path_to_id.get(short_type_path).and_then(|id| self.registrations.get(id))
    }
    ...
    pub fn get_with_short_type_path_mut(
        ...
        self.short_path_to_id.get(short_type_path).and_then(|id| self.registrations.get_mut(id))
    }
    ...
    pub fn get_type_data<T: TypeData>(&self, type_id: TypeId) -> Option<&T> {
        self.get(type_id).and_then(|registration| registration.data::<T>())
    }
    ...
    pub fn get_type_data_mut<T: TypeData>(&mut self, type_id: TypeId) -> Option<&mut T> {
        self.get_mut(type_id).and_then(|registration| registration.data_mut::<T>())
    }
...
///
/// ```
/// # use obel_reflect::{TypeRegistration, std_traits::ReflectDefault, FromType};
...
impl Debug for TypeRegistration {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("TypeRegistration").field("type_info", &self.type_info).finish()
    }
}
...
impl TypeRegistration {
    ...
    pub fn data<T: TypeData>(&self) -> Option<&T> {
        self.data.get(&TypeId::of::<T>()).and_then(|value| value.downcast_ref())
    }
    ...
    pub fn data_mut<T: TypeData>(&mut self) -> Option<&mut T> {
        self.data.get_mut(&TypeId::of::<T>()).and_then(|value| value.downcast_mut())
    }
    ...
    pub fn iter_mut(&mut self) -> impl ExactSizeIterator<Item = (TypeId, &mut dyn TypeData)> {
        self.data.iter_mut().map(|(id, data)| (*id, data.deref_mut()))
    }
}
...
/// # Example
/// ```
/// use obel_reflect::{TypeRegistry, Reflect, ReflectFromPtr};
/// use obel_platform::utils::Ptr;
...
#[expect(unsafe_code, reason = "We must interact with pointers here, which are inherently unsafe.")]
impl ReflectFromPtr {
    ...
    fn test_reflect_from_ptr() {
        ...
        };
        {
            ...
            match dyn_reflect.reflect_mut() {
                obel_reflect::ReflectMut::Struct(strukt) => {
...

        {
            ...
            match dyn_reflect.reflect_ref() {
                obel_reflect::ReflectRef::Struct(strukt) => {
                    let a = strukt.field("a").unwrap().try_downcast_ref::<f32>().unwrap();
                    assert_eq!(*a, 2.0);
                }
                _ => panic!("invalid reflection"),
            }
