...
///
/// ```
/// # use obel_reflect::func::{ArgList, DynamicFunction, IntoFunction};
/// #
...
impl<'env> DynamicFunction<'env> {
    ...
    pub fn new<F: for<'a> Fn(ArgList<'a>) -> FunctionResult<'a> + Send + Sync + 'env>(
        ...
        #[expect(unsafe_code, reason = "unsized coercion is an unstable feature for non-std types")]
        // SAFETY:
    ...
    /// ```
    /// # use std::ops::Add;
    /// # use obel_reflect::func::{ArgList, IntoFunction};
    /// #
    ...
    ///
    ///```should_panic
    /// # use obel_reflect::func::IntoFunction;
    ///
    /// fn add(a: i32, b: i32) -> i32 {
    ///     a + b
    /// }
    ...
    ///
    /// ```
    /// # use obel_reflect::func::{IntoFunction, ArgList};
    /// let c = 23;
...
impl_type_path!((in obel_reflect) DynamicFunction<'env>);
...
mod tests {
    ...
    use core::ops::Add;
    use obel_platform::collections::HashSet;
    ...
    fn should_allow_recursive_dynamic_function() {
        let factorial = DynamicFunction::new(
                ...
                match this.reflect_ref() {
                    ReflectRef::Function(func) => {
                        let result = func.reflect_call(
                            ArgList::new().with_ref(this.as_partial_reflect()).with_owned(curr - 1),
                        );
            ...
            SignatureInfo::anonymous().with_arg::<i32>("curr").with_arg::<()>("this"),
        );
    ...
    fn should_allow_variable_arguments_via_overloading() {
        ...
        let args = ArgList::default().with_owned(25_i32).with_owned(75_i32).with_owned(100_i32);
        let result = func.call(args).unwrap().unwrap_owned();
        assert_eq!(result.try_take::<i32>().unwrap(), 200);
    }
    ...
    fn should_debug_dynamic_function() {
        ...
        assert_eq!(
            debug,
            "DynamicFunction(fn obel_reflect::func::dynamic_function::tests::should_debug_dynamic_function::greet(_: &alloc::string::String) -> alloc::string::String)"
        );
    }
    ...
    fn should_debug_overloaded_dynamic_function() {
        ...
        let func = add::<i32>.into_function().with_overload(add::<f32>).with_name("add");
