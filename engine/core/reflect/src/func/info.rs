...
impl FunctionInfo {
    ...
    pub fn try_from_iter(
        ...
        if base.arg_count() > ArgCount::MAX_COUNT {
            return Err(FunctionOverloadError::TooManyArguments(ArgumentSignature::from(&base)));
        }
        ...
        for signature in iter {
            if signature.arg_count() > ArgCount::MAX_COUNT {
                return Err(FunctionOverloadError::TooManyArguments(ArgumentSignature::from(
                    &signature,
                )));
            }
    ...
    pub fn with_overload(mut self, signature: SignatureInfo) -> Result<Self, SignatureInfo> {
        ...
        self.signatures = IntoIterator::into_iter(self.signatures).chain(Some(signature)).collect();
        Ok(self)
    }
    ...
    ///
    /// ```
    /// # use obel_reflect::func::{FunctionInfo, TypedFunction};
    /// #
    ...
    pub(super) fn extend_unchecked(&mut self, other: FunctionInfo) {
        ...
        self.signatures = IntoIterator::into_iter(signatures)
            ...
            .collect();
        self.arg_count = self.signatures.iter().fold(ArgCount::default(), |mut count, sig| {
...
///
/// ```
/// # use obel_reflect::func::{FunctionInfo, PrettyPrintFunctionInfo, TypedFunction};
/// #
/// fn add(a: i32, b: i32) -> i32 {
///     a + b
/// }
...
///
/// ```
/// # use obel_reflect::func::{FunctionInfo, PrettyPrintSignatureInfo, TypedFunction};
/// #
/// fn add(a: i32, b: i32) -> i32 {
///     a + b
/// }
...
///
/// ```
/// # use obel_reflect::func::{ArgList, ReflectFnMut, TypedFunction};
/// #
...
mod tests {
    ...
    fn should_create_function_info() {
        ...
        assert_eq!(
            ...
            "obel_reflect::func::info::tests::should_create_function_info::add"
        );
    ...
    fn should_create_anonymous_function_info() {
        ...
        assert_eq!(
            core::any::type_name_of_val(&add),
            "obel_reflect::func::info::tests::should_create_anonymous_function_info::{{closure}}"
        );
    ...
    fn should_create_closure_info() {
        ...
        assert_eq!(
            core::any::type_name_of_val(&add),
            "obel_reflect::func::info::tests::should_create_closure_info::{{closure}}"
        );
