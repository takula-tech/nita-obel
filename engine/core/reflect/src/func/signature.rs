...
use obel_platform::collections::Equivalent;
...
impl<T: Borrow<SignatureInfo>> From<T> for ArgumentSignature {
    fn from(info: T) -> Self {
        Self(info.borrow().args().iter().map(ArgInfo::ty).copied().collect())
    }
}
...
mod tests {
    ...
    fn should_debug_signature() {
        ...
        assert_eq!(format!("{:?}", signature), "(&mut alloc::string::String, i32) -> ()");
    }
}
