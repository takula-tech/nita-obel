...
impl<'a> Arg<'a> {
    ...
    pub fn take_ref<T: Reflect + TypePath>(self) -> Result<&'a T, ArgError> {
        match self.value {
            ...
            ArgValue::Ref(arg) => {
                Ok(arg.try_downcast_ref().ok_or_else(|| ArgError::UnexpectedType {
                    index: self.index,
                    expected: alloc::borrow::Cow::Borrowed(T::type_path()),
                    received: alloc::borrow::Cow::Owned(arg.reflect_type_path().to_string()),
                })?)
    ...
    pub fn take_mut<T: Reflect + TypePath>(self) -> Result<&'a mut T, ArgError> {
        match self.value {
            ...
            ArgValue::Mut(arg) => {
                ...
                Ok(arg.try_downcast_mut().ok_or_else(|| ArgError::UnexpectedType {
                    index: self.index,
                    expected: alloc::borrow::Cow::Borrowed(T::type_path()),
                    received,
                })?)
    ...
    pub fn is<T: TypePath>(&self) -> bool {
        self.value.try_as_reflect().map(<dyn Reflect>::is::<T>).unwrap_or_default()
    }
}
