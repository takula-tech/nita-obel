...
impl FunctionRegistry {
    ...
    ///
    /// ```
    /// # use obel_reflect::func::{FunctionRegistrationError, FunctionRegistry};
    ...
    ///
    /// ```
    /// # use obel_reflect::func::{FunctionRegistrationError, FunctionRegistry, IntoFunction};
    /// fn add(a: i32, b: i32) -> i32 {
    ///     a + b
    /// }
    ///
        ...
        F: IntoFunction<'static, Marker> + 'static,
    {
        ...
        let name = function.name().ok_or(FunctionRegistrationError::MissingName)?.clone();
        self.functions
    ...
    ///
    /// ```should_panic
    /// # use obel_reflect::func::FunctionRegistry;
    /// fn one() {}
    /// fn two() {}
    ///
    /// let mut registry = FunctionRegistry::default();
...
mod tests {
    ...
    fn should_allow_overwriting_registration() {
        ...
        let mut registry = FunctionRegistry::default();
        registry.register(foo).unwrap();
        registry.overwrite_registration(bar.into_function().with_name(name)).unwrap();
    ...
    fn should_call_function_via_registry() {
        ...
        let result = registry.call(core::any::type_name_of_val(&add), args).unwrap();
