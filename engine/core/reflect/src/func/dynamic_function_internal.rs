...
impl<F> DynamicFunctionInternal<F> {
    ...
    pub fn new(func: F, info: FunctionInfo) -> Self {
        let arg_map =
            info.signatures().iter().map(|sig| (ArgumentSignature::from(sig), 0)).collect();
    ...
    pub fn get(&self, args: &ArgList) -> Result<&F, FunctionError> {
        ...
        self.arg_map.get(&signature).map(|index| &self.functions[*index]).ok_or_else(|| {
    ...
    pub fn get_mut(&mut self, args: &ArgList) -> Result<&mut F, FunctionError> {
        ...
        let signature = ArgListSignature::from(args);
        self.arg_map.get(&signature).map(|index| &mut self.functions[*index]).ok_or_else(|| {
            FunctionError::NoOverload {
                expected: self.arg_map.keys().cloned().collect(),
                received: ArgumentSignature::from(args),
            }
...
impl<F> Debug for DynamicFunctionInternal<F> {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        self.info.pretty_printer().include_fn_token().include_name().fmt(f)
    }
}
...
mod tests {
    ...
    fn should_return_error_on_duplicate_signature() {
        let mut func_a = DynamicFunctionInternal::new(
            'a',
            FunctionInfo::new(
                SignatureInfo::anonymous().with_arg::<i8>("arg0").with_arg::<i16>("arg1"),
            ),
        );
        ...
        let func_b = DynamicFunctionInternal {
            ...
            info: FunctionInfo::new(
                SignatureInfo::anonymous().with_arg::<u8>("arg0").with_arg::<u16>("arg1"),
            )
            .with_overload(
                SignatureInfo::anonymous().with_arg::<i8>("arg0").with_arg::<i16>("arg1"),
            )
            .unwrap(),
            arg_map: HashMap::from_iter([
                (ArgumentSignature::from_iter([Type::of::<u8>(), Type::of::<u16>()]), 0),
                (ArgumentSignature::from_iter([Type::of::<i8>(), Type::of::<i16>()]), 1),
            ]),
        };
        ...
        assert_eq!(duplicate, ArgumentSignature::from_iter([Type::of::<i8>(), Type::of::<i16>()]));
