...
pub(crate) fn generate_generics(meta: &ReflectMeta) -> Option<TokenStream> {
    ...
    let generics = meta
        ...
        .filter_map(|param| match param {
            GenericParam::Type(ty_param) => {
                ...
                Some(quote! {
                    #obel_reflect_path::GenericInfo::Type(
                        #obel_reflect_path::TypeParamInfo::new::<#ident>(
                            #obel_reflect_path::__macro_exports::alloc_utils::Cow::Borrowed(#name),
                        )
                        #with_default
                    )
                })
            ...
            GenericParam::Const(const_param) => {
                ...
                Some(quote! {
                    ...
                    #obel_reflect_path::GenericInfo::Const(
                        #obel_reflect_path::ConstParamInfo::new::<#ty>(
                            #obel_reflect_path::__macro_exports::alloc_utils::Cow::Borrowed(#name),
                        )
                        #with_default
                    )
    ...
    Some(quote!(#obel_reflect_path::Generics::from_iter([ #generics ])))
}
