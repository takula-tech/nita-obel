...
fn static_type_cell(
    ...
    if meta.type_path().impl_is_generic() {
        ...
        quote! {
            static CELL: #obel_reflect_path::utility::#cell_type = #obel_reflect_path::utility::#cell_type::new();
...
pub(crate) fn impl_type_path(meta: &ReflectMeta) -> TokenStream {
    ...
    let (long_type_path, short_type_path) = if type_path.impl_is_generic() {
        let long_path_cell = static_type_cell(
            ...
            type_path.long_type_path(obel_reflect_path).into_owned(),
        );
        let short_path_cell = static_type_cell(
            meta,
            TypedProperty::TypePath,
            type_path.short_type_path(obel_reflect_path).into_owned(),
        );
        (long_path_cell.to_token_stream(), short_path_cell.to_token_stream())
    } else {
        (
            type_path.long_type_path(obel_reflect_path).into_borrowed(),
            type_path.short_type_path(obel_reflect_path).into_borrowed(),
        )
    };
    ...
    quote! {
        ...
        const _: () = {
            ...
            impl #impl_generics #obel_reflect_path::TypePath for #type_path #ty_generics #where_reflect_clause {
...
pub(crate) fn impl_typed(
    ...
    quote! {
        impl #impl_generics #obel_reflect_path::Typed for #type_path #ty_generics #where_reflect_clause {
            #[inline]
            fn type_info() -> &'static #obel_reflect_path::TypeInfo {
                #type_info_cell
            }
        }
    }
