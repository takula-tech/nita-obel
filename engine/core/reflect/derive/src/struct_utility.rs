...
impl FieldAccessors {
    pub fn new(reflect_struct: &ReflectStruct) -> Self {
        ...
        let fields_ref =
            Self::get_fields(reflect_struct, |field, accessor| match &field.attrs.remote {
                Some(wrapper_ty) => {
                    quote! {
                        <#wrapper_ty as #obel_reflect_path::ReflectRemote>::as_wrapper(&#accessor)
                    }
                }
                None => quote!(& #accessor),
            });
        let fields_mut = Self::get_fields(reflect_struct, |field, accessor| {
            match &field.attrs.remote {
                Some(wrapper_ty) => {
                    quote! {
                        <#wrapper_ty as #obel_reflect_path::ReflectRemote>::as_wrapper_mut(&mut #accessor)
                    }
                }
