...
use super::generics::generate_generics;
use syn::{
...
impl<'a> ReflectStruct<'a> {
    ...
    pub fn to_info_tokens(&self, is_tuple: bool) -> proc_macro2::TokenStream {
        ...
        let field_infos = self.active_fields().map(|field| field.to_info_tokens(bevy_reflect_path));

        let custom_attributes = self.meta.attrs.custom_attributes().to_tokens(bevy_reflect_path);
...
impl<'a> ReflectEnum<'a> {
    ...
    pub fn to_info_tokens(&self) -> proc_macro2::TokenStream {
        ...
        let variants =
            self.variants.iter().map(|variant| variant.to_info_tokens(bevy_reflect_path));
...
impl<'a> EnumVariant<'a> {
    ...
    pub fn to_info_tokens(&self, bevy_reflect_path: &Path) -> proc_macro2::TokenStream {
        ...
        let fields = self.active_fields().map(|field| field.to_info_tokens(bevy_reflect_path));
...
pub(crate) enum ReflectTypePath<'a> {
    ...
    Primitive(&'a Ident),
    /// Using `::my_crate::derive::foo::Bar` syntax.
    ///
