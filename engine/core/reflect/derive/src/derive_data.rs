...
pub(crate) struct ReflectMeta<'a> {
    ...
    /// A cached instance of the path to the `obel_reflect` crate.
...
impl<'a> ReflectDerive<'a> {
        ...
        match &input.data {
            Data::Struct(data) => {
                ...
                let serialization_data =
                    SerializationDataDef::new(&fields, &meta.obel_reflect_path)?;
            ...
            Data::Union(..) => {
                Err(syn::Error::new(input.span(), "reflection not supported for unions"))
            }
        }
    }
    ...
    fn collect_struct_fields(fields: &'a Fields) -> Result<Vec<StructField<'a>>, syn::Error> {
        ...
        let sifter: ResultSifter<StructField<'a>> = fields
            .iter()
            .enumerate()
            .map(|(declaration_index, field)| -> Result<StructField, syn::Error> {
...
impl<'a> ReflectMeta<'a> {
    ...
    /// The cached `obel_reflect` path.
    pub fn obel_reflect_path(&self) -> &Path {
        &self.obel_reflect_path
    }
...
impl StructField<'_> {
    ...
    pub fn to_info_tokens(&self, obel_reflect_path: &Path) -> proc_macro2::TokenStream {
        ...
        let field_info = if self.data.ident.is_some() {
            quote! {
                #obel_reflect_path::NamedField
            }
        } else {
            quote! {
                #obel_reflect_path::UnnamedField
            }
        };

        let ty = self.reflected_type();
        let custom_attributes = self.attrs.custom_attributes.to_tokens(obel_reflect_path);
...
impl<'a> ReflectStruct<'a> {
    ...
    pub fn active_types(&self) -> Vec<Type> {
        self.active_fields().map(|field| field.reflected_type().clone()).collect()
    }

    /// Get an iterator of fields which are exposed to the reflection API.
    pub fn active_fields(&self) -> impl Iterator<Item = &StructField<'a>> {
        self.fields().iter().filter(|field| field.attrs.ignore.is_active())
    }

    /// Get an iterator of fields which are ignored by the reflection API
    pub fn ignored_fields(&self) -> impl Iterator<Item = &StructField<'a>> {
        self.fields().iter().filter(|field| field.attrs.ignore.is_ignored())
    }
    ...
    pub fn to_info_tokens(&self, is_tuple: bool) -> proc_macro2::TokenStream {
        ...
        let (info_variant, info_struct) = if is_tuple {
            ...
            (Ident::new("Struct", Span::call_site()), Ident::new("StructInfo", Span::call_site()))
        };

        let field_infos = self.active_fields().map(|field| field.to_info_tokens(obel_reflect_path));

        let custom_attributes = self.meta.attrs.custom_attributes().to_tokens(obel_reflect_path);

        let mut info = quote! {
            #obel_reflect_path::#info_struct::new::<Self>(&[
                #(#field_infos),*
            ])
...
impl<'a> ReflectEnum<'a> {
    ...
    pub fn to_info_tokens(&self) -> proc_macro2::TokenStream {
        ...
        let variants =
            self.variants.iter().map(|variant| variant.to_info_tokens(obel_reflect_path));

        let custom_attributes = self.meta.attrs.custom_attributes().to_tokens(obel_reflect_path);

        let mut info = quote! {
            #obel_reflect_path::EnumInfo::new::<Self>(&[
                #(#variants),*
            ])
            .with_custom_attributes(#custom_attributes)
        };
...
impl<'a> EnumVariant<'a> {
    ...
    pub fn to_info_tokens(&self, obel_reflect_path: &Path) -> proc_macro2::TokenStream {
        ...
        let fields = self.active_fields().map(|field| field.to_info_tokens(obel_reflect_path));
        ...
        let mut info = quote! {
            #obel_reflect_path::#info_struct::new(#args)
                .with_custom_attributes(#custom_attributes)
        };
        ...
        quote! {
            #obel_reflect_path::VariantInfo::#info_variant(#info)
        }
    }
}
...
/// ```ignore  (obel_reflect is not accessible from this crate)
/// # use syn::parse_quote;
/// # use obel_reflect_derive::ReflectTypePath;
...
impl<'a> ReflectTypePath<'a> {
    ...
    pub fn get_ident(&self) -> Option<&Ident> {
        match self {
            ...
            Self::External {
                ...
                custom_path,
                ..
            } => Some(&custom_path.as_ref().unwrap_or(path).segments.last().unwrap().ident),
    ...
    pub fn impl_is_generic(&self) -> bool {
        ...
        !self.generics().params.iter().all(|param| matches!(param, GenericParam::Lifetime(_)))
    }
    ...
    pub fn long_type_path(&self, obel_reflect_path: &Path) -> StringExpr {
        match self {
            Self::Primitive(ident) => StringExpr::from(ident),
            Self::Anonymous {
                long_type_path,
                ..
            } => long_type_path.clone(),
            Self::Internal {
                generics,
                ..
            }
                    ...
                    let generics = ReflectTypePath::reduce_generics(
                        ...
                        |TypeParam {
                            ...
                            StringExpr::Borrowed(quote! {
                                <#ident as #obel_reflect_path::TypePath>::type_path()
                            })
                        },
                        obel_reflect_path,
                    );
    ...
    pub fn short_type_path(&self, obel_reflect_path: &Path) -> StringExpr {
        match self {
            Self::Anonymous {
                short_type_path,
                ..
                ...
                if self.impl_is_generic() {
                    let generics = ReflectTypePath::reduce_generics(
                        ...
                        |TypeParam {
                            ...
                            StringExpr::Borrowed(quote! {
                                <#ident as #obel_reflect_path::TypePath>::short_type_path()
                            })
                        },
                        obel_reflect_path,
                    );
    ...
    pub fn true_type(&self) -> proc_macro2::TokenStream {
        match self {
            ...
            Self::Anonymous {
                qualified_type,
                ..
            } => qualified_type.to_token_stream(),
        }
    }
}
...
impl ToTokens for ReflectTypePath<'_> {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        match self {
            ...
            | Self::Primitive(ident) => ident.to_tokens(tokens),
            Self::External {
                path,
                ..
            ...
            Self::Anonymous {
                qualified_type,
                ..
            } => qualified_type.to_tokens(tokens),
        }
    }
}
