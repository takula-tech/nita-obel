...
pub(crate) trait VariantBuilder: Sized {
            ...
            None => {
                if let Some(field_index) = field.field.reflection_index {
                    ...
                    quote!(::core::compile_error!(
                        "internal obel_reflect error: field should be active"
                    ))
                }
            }
        }
    ...
    fn on_active_field(&self, this: &Ident, field: VariantField) -> TokenStream {
        ...
        if field.field.attrs().remote.is_some() {
            quote! {
                <#field_ty as #obel_reflect_path::ReflectRemote>::into_remote(#construction)
            }
        } else {
            construction
        }
...
impl VariantBuilder for FromReflectVariantBuilder<'_> {
    ...
    fn construct_field(&self, field: VariantField) -> TokenStream {
        ...
        quote! {
            <#field_ty as #obel_reflect_path::FromReflect>::from_reflect(#alias)?
        }
    }
}
...
impl VariantBuilder for TryApplyVariantBuilder<'_> {
    ...
    fn unwrap_field(&self, field: VariantField) -> TokenStream {
        ...
        quote! {
            #alias.ok_or(#obel_reflect_path::ApplyError::MissingEnumField {
    ...
    fn construct_field(&self, field: VariantField) -> TokenStream {
        ...
        quote! {
            <#field_ty as #obel_reflect_path::FromReflect>::from_reflect(#alias)
                .ok_or(#obel_reflect_path::ApplyError::MismatchedTypes {
                    from_type: ::core::convert::Into::into(
                        #obel_reflect_path::DynamicTypePath::reflect_type_path(#alias)
                    ),
