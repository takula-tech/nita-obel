...
pub(crate) fn impl_opaque(meta: &ReflectMeta) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl #impl_generics #obel_reflect_path::FromReflect for #type_path #ty_generics #where_from_reflect_clause  {
            fn from_reflect(reflect: &dyn #obel_reflect_path::PartialReflect) -> #FQOption<Self> {
                #FQOption::Some(
                    #FQClone::clone(
                        <dyn #obel_reflect_path::PartialReflect>::try_downcast_ref::<#type_path #ty_generics>(reflect)?
                    )
                )
            }
        }
...
pub(crate) fn impl_enum(reflect_enum: &ReflectEnum) -> proc_macro2::TokenStream {
    ...
    let where_from_reflect_clause =
        reflect_enum.where_clause_options().extend_where_clause(where_clause);

    quote! {
        impl #impl_generics #obel_reflect_path::FromReflect for #enum_path #ty_generics #where_from_reflect_clause  {
            fn from_reflect(#ref_value: &dyn #obel_reflect_path::PartialReflect) -> #FQOption<Self> {
                    ...
                    #obel_reflect_path::PartialReflect::reflect_ref(#ref_value)
                {
                    match #obel_reflect_path::Enum::variant_name(#ref_value) {
                        #match_branches
                        name => panic!("variant with name `{}` does not exist on enum `{}`", name, <Self as #obel_reflect_path::TypePath>::type_path()),
                    }
                } else {
                    #FQOption::None
                }
...
fn impl_struct_internal(
    ...
    let (reflect_ty, constructor, retval) = if let Some(remote_ty) = remote_ty {
        ...
        (quote!(#remote_ty), quote!(#constructor), quote!(Self(#__this)))
    } else {
    ...
    // Add FromReflect bound for each active field
    let where_from_reflect_clause =
        reflect_struct.where_clause_options().extend_where_clause(where_clause);

    quote! {
        impl #impl_generics #obel_reflect_path::FromReflect for #struct_path #ty_generics #where_from_reflect_clause {
            fn from_reflect(reflect: &dyn #obel_reflect_path::PartialReflect) -> #FQOption<Self> {
                if let #obel_reflect_path::ReflectRef::#ref_struct_type(#ref_struct)
    ...
    MemberValuePair::new(
        reflect_struct
            ...
            .map(|field| {
                ...
                let get_field = quote! {
                    #obel_reflect_path::#struct_type::field(#dyn_struct_name, #accessor)
                };
                ...
                let value = match &field.attrs.default {
                    DefaultBehavior::Func(path) => {
                        let value = into_remote(quote! {
                            <#ty as #obel_reflect_path::FromReflect>::from_reflect(field)
                        });
                        quote! {
                            (||
                    ...
                    DefaultBehavior::Required => {
                        let value = into_remote(quote! {
                            <#ty as #obel_reflect_path::FromReflect>::from_reflect(#get_field?)
                        });
                        quote! {
                            (|| #value)
                        }
