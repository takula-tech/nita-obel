...
use attr::container::ContainerAttributes;
...
/// The main derive macro used by `obel_reflect` for deriving its `Reflect` trait.
///
...
///   `(&self, value: &dyn #obel_reflect_path::Reflect) -> bool`.
...
/// // impl obel_reflect::Reflect for Foo
/// // where
/// //   Self: Any + Send + Sync,
...
/// // Generates a where clause like:
/// //
/// // impl<T: Trait> obel_reflect::Reflect for Foo<T>
/// // where
/// //   Self: Any + Send + Sync,
/// //   T::Assoc: Default,
/// //   T: TypePath,
...
/// ```ignore (obel_reflect is not accessible from this crate)
/// # use std::any::TypeId;
/// # use obel_reflect_derive::{Reflect, reflect_trait};
...
///
/// The only reason for this macro's existence is so that `obel_reflect` can easily implement the reflection traits
...
/// variant traits (`obel_reflect::{Struct, TupleStruct, Enum}`, etc.),
/// which have greater functionality. The type being reflected must be in scope, as you cannot
/// qualify it in the macro as e.g. `obel::prelude::Vec3`.
///
...
///
/// # Example
/// Implementing `Reflect` for `obel::prelude::Vec3` as a struct type:
/// ```ignore (obel_reflect is not accessible from this crate)
/// use obel::prelude::Vec3;
///
/// impl_reflect!(
///     #[reflect(PartialEq, Serialize, Deserialize, Default)]
///     #[type_path = "obel::prelude"]
///     struct Vec3 {
///         x: f32,
///         y: f32,
///         z: f32
...
///
/// The only reason this macro exists is so that `obel_reflect` can easily implement `FromReflect` on
