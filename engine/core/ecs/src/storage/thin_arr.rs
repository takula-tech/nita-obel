#![expect(
    unsafe_code,
    reason = "ThinArrayPtr apparently unsafe with use of NonNull, *_unchecked and more"
)]

use crate::DebugCheckedUnwrap;
use alloc::{
...
impl<T> ThinArrayPtr<T> {
    ...
    pub unsafe fn swap_remove_unchecked_nonoverlapping(
        ...
        let value = unsafe { ptr::read(base_ptr.add(index_to_remove)) };
        unsafe {
            ptr::copy_nonoverlapping(base_ptr.add(index_to_keep), base_ptr.add(index_to_remove), 1)
        };
        value
    }
    ...
    pub unsafe fn swap_remove_unchecked(
        ...
        unsafe { ptr::read(self.data.as_ptr().add(index_to_remove)) }
    }
    ...
    pub unsafe fn swap_remove_and_drop_unchecked(
        ...
        unsafe { ptr::drop_in_place(ptr::from_mut(val)) };
    }
    ...
    unsafe fn last_element(&mut self, current_len: usize) -> Option<*mut T> {
        (current_len != 0).then_some(unsafe { self.data.as_ptr().add(current_len - 1) })
    }
    ...
    pub unsafe fn clear_elements(&mut self, mut current_len: usize) {
        if needs_drop::<T>() {
            while let Some(to_drop) = unsafe { self.last_element(current_len) } {
    ...
    pub unsafe fn drop(&mut self, current_capacity: usize, current_len: usize) {
        ...
        if current_capacity != 0 {
            unsafe { self.clear_elements(current_len) };
