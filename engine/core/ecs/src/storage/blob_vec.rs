#![expect(
    unsafe_code,
    reason = "BlobVec apparently unsafe with use of NonNull, *_unchecked and more"
)]

use alloc::alloc::handle_alloc_error;
use core::{alloc::Layout, cell::UnsafeCell, num::NonZero, ptr::NonNull};
use obel_platform::utils::{OnDrop, OwningPtr, Ptr, PtrMut, dangling_with_align};

/// A flat, type-erased data storage type
///
/// Used to densely store homogeneous ECS data.
/// A blob is usually just an arbitrary block of contiguous memory without any identity,
/// and could be used to represent any arbitrary data (i.e. string, arrays, etc).
/// This type is an extendable and re-allocatable blob, which makes it a blobby Vec, a `BlobVec`.
...
impl BlobVec {
    ...
    pub unsafe fn new(
        ...
        let data = dangling_with_align(align);
    ...
    fn grow_exact(&mut self, increment: NonZero<usize>) {
        let new_capacity = self.capacity.checked_add(increment.get()).expect("capacity overflow");
        let new_layout =
...
pub(super) unsafe fn array_layout_unchecked(layout: &Layout, n: usize) -> Layout {
    let (array_layout, offset) = unsafe { repeat_layout_unchecked(layout, n) };
    debug_assert_eq!(layout.size(), offset);
    array_layout
}
...
unsafe fn repeat_layout_unchecked(layout: &Layout, n: usize) -> (Layout, usize) {
    ...
    // SAFETY: self.align is already known to be valid and alloc_size has been
    // padded already.
    unsafe { (Layout::from_size_align_unchecked(alloc_size, layout.align()), padded_size) }
}
...
mod tests {
    ...
    unsafe fn swap_remove<T>(blob_vec: &mut BlobVec, index: usize) -> T {
        ...
        let value = unsafe { blob_vec.swap_remove_and_forget_unchecked(index) };
        unsafe { value.read::<T>() }
    }
    ...
    unsafe fn get_mut<T>(blob_vec: &mut BlobVec, index: usize) -> &mut T {
        assert!(index < blob_vec.len());
        unsafe { blob_vec.get_unchecked_mut(index).deref_mut::<T>() }
    }
    ...
    //         assert_eq!(core::ptr::from_ref::<Zst>(zst) as usize % align_of::<Zst>(), 0);
    //         count += 1;
    //     }
