#![expect(
    unsafe_code,
    reason = "BlobArray apparently unsafe with use of NonNull, *_unchecked and more"
)]
...
impl BlobArray {
    ...
    pub unsafe fn initialize_unchecked(&mut self, index: usize, value: OwningPtr<'_>) {
        ...
        let size = self.item_layout.size();
        let dst = unsafe { self.get_unchecked_mut(index) };
        unsafe { core::ptr::copy::<u8>(value.as_ptr(), dst.as_ptr(), size) };
    }
    ...
    pub unsafe fn swap_remove_unchecked(
        ...
        if index_to_remove != index_to_keep {
            return unsafe {
                self.swap_remove_unchecked_nonoverlapping(index_to_remove, index_to_keep)
            };
        }
        ...
        unsafe { self.get_unchecked_mut(index_to_keep).promote() }
    }
    ...
    pub unsafe fn swap_remove_and_drop_unchecked(
        ...
        let drop = self.drop;
        let value = unsafe { self.swap_remove_unchecked(index_to_remove, index_to_keep) };
    ...
    pub unsafe fn swap_remove_and_drop_unchecked_nonoverlapping(
        ...
        let drop = self.drop;
        let value =
            unsafe { self.swap_remove_unchecked_nonoverlapping(index_to_remove, index_to_keep) };
        if let Some(drop) = drop {
            unsafe { drop(value) };
        }
    }
