...
use std::{collections::HashSet, format, string::ToString, vec::Vec};
use syn::{
    Data, DataEnum, DataStruct, DeriveInput, Expr, ExprCall, ExprClosure, ExprPath, Field, Fields,
    Ident, LitStr, Member, Path, Result, Token, Type, Visibility, parenthesized,
    parse::Parse,
...
pub fn derive_event(mut ast: DeriveInput) -> TokenStream {
...
pub fn derive_resource(mut ast: DeriveInput) -> TokenStream {
    // let mut ast = parse_macro_input!(input as DeriveInput);
    let obel_ecs_path: Path = crate::obel_ecs_path();
...
pub fn derive_component(mut ast: DeriveInput) -> TokenStream {
    // let mut ast = parse_macro_input!(input as DeriveInput);
    let obel_ecs_path: Path = crate::obel_ecs_path();
    ...
    quote! {
        ...
        impl #impl_generics #obel_ecs_path::component::Component for #struct_name #type_generics #where_clause {
            ...
            fn register_required_components(
                ...
                components: &mut #obel_ecs_path::component::Components,
...
impl HookAttributeKind {
    ...
    fn to_token_stream(&self, obel_ecs_path: &Path) -> TokenStream {
        match self {
...
fn storage_path(obel_ecs_path: &Path, ty: StorageTy) -> TokenStream {
...
mod tests {
    ...
    fn test_derive_resource() {
        ...
        let output = derive_resource(input);
        let expected = quote! {
    ...
    fn test_derive_component() {
        ...
        let output = derive_component(input);
        let expected = quote! {
            impl obel_ecs::component::Component for MyComponent where Self : Send + Sync + 'static {
                ...
                fn register_required_components(
                    ...
                    components: &mut obel_ecs::component::Components,
                    required_components: &mut obel_ecs::component::RequiredComponents,
                    inheritance_depth: u16,
                    recursion_check_stack: &mut obel_ecs::__macro_exports::Vec<obel_ecs::component::ComponentId>
                ) {
                    obel_ecs::component::enforce_no_required_components_recursion(components, recursion_check_stack);
                    let self_id = components.register_component::<Self>();
                    recursion_check_stack.push(self_id);
                    recursion_check_stack.pop();
                }
                ...
                fn clone_behavior() -> obel_ecs::component::ComponentCloneBehavior {
                    use obel_ecs::component::{DefaultCloneBehaviorBase, DefaultCloneBehaviorViaClone};
                    (&&&obel_ecs::component::DefaultCloneBehaviorSpecialization::<Self>::default()).default_clone_behavior()
                }
            }
        };
