...
ptr_shared_impl!(Ptr);
ptr_shared_impl!(PtrMut);
ptr_shared_impl!(OwningPtr);
...
impl OwningPtr<'_> {
    ...
    unsafe fn make_internal<T>(temp: &mut ManuallyDrop<T>) -> OwningPtr {
...
pub trait UnsafeCellDeref<'a, T>: sealed::SealedUnsafeCell {
    /// # Safety
...
impl<T: Sized> DebugEnsureAligned for *mut T {
    // Without #[track_caller], if the assertion fails,
    // the error would point to the line inside debug_ensure_aligned where the assert happens - assert_eq!(...)
    // With #[track_caller], the error will instead point to where the misaligned pointer
    // was actually created/used in the calling code
    #[track_caller]
    fn debug_ensure_aligned(self) -> Self {
