...
#[expect(
    unsafe_code,
    reason = "SAFETY: `T` is Sync, caller is responsible for upholding rust safety rules"
)]
...
impl<T: ?Sized> SyncUnsafeCell<T> {
    ...
    pub fn from_mut(t: &mut T) -> &mut SyncUnsafeCell<T> {
        ...
        #[expect(
            unsafe_code,
            reason = "SAFETY:
            `ptr` must be safe to mutably dereference, since it was originally obtained from a mutable reference. `SyncUnsafeCell` has the same representation as the original type `T`, since the former is annotated with #[repr(transparent)]."
        )]
        unsafe {
            &mut *ptr
        }
...
impl<T> SyncUnsafeCell<[T]> {
    ...
    pub fn as_slice_of_cells(&self) -> &[SyncUnsafeCell<T>] {
        ...
        #[expect(
            unsafe_code,
            reason = "SAFETY:
            `UnsafeCell<T>` and `SyncUnsafeCell<T>` have #[repr(transparent)], Therefore:
