//! Provide ui test runner function to ease running ui tests.
...
fn basic_config(root_dir: impl Into<PathBuf>, args: &Args) -> ui_test::Result<Config> {
    ...
    config
        ...
        .base()
        .custom
        .insert("dependencies", Spanned::dummy(vec![Box::new(DependencyBuilder::default())]));
...
pub fn test_with_multiple_configs(
    ...
    run_tests_generic(configs, default_file_filter, default_per_file_config, emitter)
}
